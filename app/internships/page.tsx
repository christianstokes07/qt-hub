...
export default function InternshipsPage() {
  ...
  const filteredFeatured = (featuredData as Featured[])
    .filter((job) => {
      ...
      const searchMatch = search === "" || job.title.toLowerCase().includes(search.toLowerCase()) || job.company.toLowerCase().includes(search.toLowerCase());
      const locationTypeMatch = locationType === "All Locations" || job.location.toLowerCase().includes(locationType.toLowerCase());
      const locLower = locationSearch.toLowerCase().trim();
      const stateCode = STATE_ALIASES[locLower];
      const locationSearchMatch = locationSearch === "" || job.location.toLowerCase().includes(locLower) || (stateCode && job.states?.includes(stateCode)) || job.states?.some((s) => s.toLowerCase().includes(locLower));
  ...
  const displayedLiveJobs = locationSearch.trim()
    ? liveJobs.filter((job) => job.location?.display_name?.toLowerCase().includes(locationSearch.toLowerCase().trim()))
    : liveJobs;
  ...
  const fetchLiveJobs = useCallback(async (query: string) => {
    ...
    try {
      ...
      const fullQuery = query ? `${query} internship${locationTypeSuffix}` : `${keyword}${locationTypeSuffix}`;
      const res = await fetch(`/api/jobs?query=${encodeURIComponent(fullQuery)}${locationSearch ? `&location=${encodeURIComponent(locationSearch)}` : ""}`);
  ...
  useEffect(() => { if (searched) fetchLiveJobs(search); }, [major, locationType]);
  useEffect(() => { fetchLiveJobs(""); }, []);

  const handleSearch = (e: React.FormEvent) => { e.preventDefault(); fetchLiveJobs(search); };
  const hasActiveFilters = major !== "All Majors" || locationType !== "All Locations" || locationSearch !== "" || search !== "";
  const clearAll = () => { setMajor("All Majors"); setLocationType("All Locations"); setLocationSearch(""); setSearch(""); fetchLiveJobs(""); };
        ...
        <div className="max-w-6xl mx-auto space-y-3">
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search internships, companies..."
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-end">
            ...
            <div className="flex flex-col gap-1 min-w-[180px]">
              ...
              <div className="relative">
                ...
                <input type="text" value={locationSearch}
                  onChange={(e) => { const val = e.target.value; setLocationSearch(val); if (val === "") fetchLiveJobs(search); }}
                  onKeyDown={(e) => { if (e.key === "Enter") fetchLiveJobs(search); }}
            ...
            {hasActiveFilters && (
              <div className="flex items-end pb-0.5">
                <button onClick={clearAll} className="text-sm text-pink-500 hover:text-pink-700 font-medium transition-colors whitespace-nowrap">Clear all ✕</button>
              </div>
            )}
          </div>
        </div>
                ...
                <div key={job.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-200 flex flex-col">
                  <div className="p-6 flex-1">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <span className="text-xs bg-pink-50 text-pink-600 px-2.5 py-1 rounded-full border border-pink-100 font-medium">{job.major[0]}</span>
                    </div>
                  </div>
