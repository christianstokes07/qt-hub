...
export default function ScholarshipsPage() {
  ...
  const filtered = (scholarshipData as Scholarship[]).filter((s) => {
    const majorMatch =
      major === "All Majors" ||
      s.major.includes("All Majors") ||
      s.major.includes(major);
      ...
      <section className="bg-white border-b border-gray-100 py-5 px-6 sticky top-[69px] z-40 shadow-sm">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row gap-3 items-stretch sm:items-end">
          ...
          <div className="flex items-end gap-3 pb-0.5">
            ...
            {(major !== "All Majors" || year !== "All Years" || search !== "") && (
              <button
                onClick={() => { setMajor("All Majors"); setYear("All Years"); setSearch(""); }}
                className="text-sm text-pink-500 hover:text-pink-700 font-medium transition-colors whitespace-nowrap"
              >
                Clear ✕
              </button>
            )}
      ...
      <section className="max-w-6xl mx-auto px-6 py-10">
        {filtered.length === 0 ? (
          <div className="text-center py-24">
            ...
            <button
              onClick={() => { setMajor("All Majors"); setYear("All Years"); setSearch(""); }}
              className="bg-pink-400 hover:bg-pink-500 text-white font-semibold px-6 py-3 rounded-full transition-colors"
            >
              Clear Filters
            </button>
          </div>
